			...
			<div class="roomlist">
				<p>
					...
					<span
						...
						title={TL`Giratina is Relumi's mascot! It is a fierce and powerful Pokémon, and represents our battles.`}
					></span>
				</p>
...
export class BattleRoom extends ChatRoom {
	...
	loadReplay() {
		...
		Net(`https://${Config.routes.replays}/battle-${replayid}.json`).get().catch(() => '').then(data => {
			try {
...
class BattlePanel extends PSRoomPanel<BattleRoom> {
	...
	updateLayout() {
		...
		const { battleHeight, bigpicture } = this.chooseLayout();
		this.battleHeight = bigpicture ? 720 : battleHeight; // battle canvas height; Big Picture is 360 * 2
		const frame = room.battle?.scene.$frame;
		if (bigpicture && frame) {
			// Big Picture Mode: scale battle canvas 2x for clean pixel-art rendering.
			// Anchor transform at top-left so the scaled canvas fills the 1280x720
			// container instead of bleeding out left/up (default transform-origin
			// is 50% 50%, which would clip the battle on narrow viewports).
			frame.css('transform', 'scale(2)');
			frame.css('transform-origin', 'left top');
			frame.css('image-rendering', 'pixelated');
		} else if (frame) {
			// Reset all bigpicture-mode styles (transform, transform-origin,
			// image-rendering) so the standard scaled path matches upstream.
			frame.css('image-rendering', '');
			frame.css('transform-origin', '');
			frame.css('transform', battleHeight !== 360 ? `scale(${battleHeight / 360})` : 'none');
		}
	}
	chooseLayout(): {
		...
		if (bigpicture) {
			return {
				...
				battleHeight: 720, // 360 * 2
				battleWidth: 1280, // 640 * 2
				overlayControls: layoutState.overlayControls,
				bigpicture: true,
			};
		}
		return {
			layout: layoutState.layout,
			battleHeight: layoutState.battleHeight,
			battleWidth: layoutState.battleWidth,
			overlayControls: layoutState.overlayControls,
			bigpicture: false,
		};
	}
	...
	override render() {
		...
		></style> : null;
		const { layout, battleHeight, battleWidth, overlayControls, bigpicture } = this.chooseLayout();
		const overlayVersion = overlayControls && !!room.battle && !!room.side && !!room.request && !room.battle.ended;
		// Big Picture Mode CSS overrides:
		// - .inline-controls becomes a flex row so the move/switch columns
		//   align horizontally across the wider 1280px battle canvas.
		//   (.inline-controls is the actual inner container; .battle-controls
		//   is the outer positioned wrapper, and .movecontrols/.switchcontrols
		//   are grandchildren of it, not direct children — using `>` on the
		//   outer wouldn't match.)
		// - .whatdo (the "What will X do?" prompt) takes the full first row.
		// - .movecontrols and .switchcontrols share row 2 with `flex: 1` and
		//   a 200px min-width so the columns never crush the move buttons.
		// - .backdrop forces smooth interpolation on photographic JPG backgrounds
		//   so the 2x pixelated scale doesn't introduce nearest-neighbor artifacts.
		const bigpictureStyle = bigpicture ? <style
			dangerouslySetInnerHTML={{ __html: [
				`#${id} .inline-controls { display: flex; flex-wrap: wrap; }`,
				`#${id} .inline-controls > .whatdo { flex: 0 0 100%; }`,
				`#${id} .inline-controls > .movecontrols,`,
				`#${id} .inline-controls > .switchcontrols { flex: 1; min-width: 200px; }`,
