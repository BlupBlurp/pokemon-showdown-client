...
export class BattleTooltips {
	...
	private renderStatWithHighlight(
		...
		const statClass = this._relumi.getStatClass(speciesId, statName, value);
		const diff = this._relumi.getStatDiff(speciesId, statName, value);
	...
	private renderAbilityWithHighlight(speciesId: ID | undefined, abilityName: string): string {
		if (speciesId && this._relumi.isNewRelumiAbility(speciesId, abilityName)) {
	...
	showMoveTooltip(
		...
		if (this.battle.hardcoreMode) {
			...
			if (priority > 1) {
				...
				// Relumi: Gale Wings grants +1 priority at any HP, not just full HP.
				const isRelumi = toID(this.battle.tier).includes("relumi");
				if (ability === 'galewings' && moveType === 'Flying' && (isRelumi || pokemon.hp === pokemon.maxhp)) {
					text += BattleTextParser.ui('usuallyMovesFirst', { PRIORITY: '1' }) + '</p><p>';
				}
			}
	...
	getPokemonAbilityText(
		...
		if (!isActive) {
			...
			if (ability) {
				const abilityName = TL(this.battle.dex.abilities.get(ability));
				text = `<small>${TL.label(TL.term.ability)}</small>` + this.renderAbilityWithHighlight(speciesId, abilityName);
			}
		} else {
			if (abilityData.ability) {
				...
				if (baseAbilityName && baseAbilityName !== abilityName) {
					text += ' ' + BattleTextParser.ui('base', { VALUE: this.renderAbilityWithHighlight(speciesId, baseAbilityName) });
				}
			}
		}
		...
		if (!text && abilityData.possibilities.length && !hidePossible &&
			...
			text = '<small>' + TL.label(TL.term.possibleabilities) + '</small>' + abilityData.possibilities
				.map(ability => this.renderAbilityWithHighlight(speciesId, TL(this.battle.dex.abilities.get(ability)))).join(', ');
		}
		return text;
	}
}
