	...
	getPokemonAbilityText(
		...
		if (!isActive) {
			...
			if (ability) {
				...
				text = `<small>${TL.label(TL`Ability`)}</small>` + this.renderAbilityWithHighlight(speciesId, abilityName);
			}
		} else {
			if (abilityData.ability) {
				...
				if (baseAbilityName && baseAbilityName !== abilityName) {
					text += ' ' + TL`(base: ${this.renderAbilityWithHighlight(speciesId, baseAbilityName)})`;
				}
			}
		}
		...
		if (!text && abilityData.possibilities.length && !hidePossible &&
			...
			let parenthetical = '';
			for (const next of abilityData.possibilities.map(ability => this.renderAbilityWithHighlight(speciesId, TL(this.battle.dex.abilities.get(ability))))) {
