...
class BattleMoveSearch extends BattleTypedSearch<"move"> {
	...
	private moveIsNotUseless(
		...
		switch (id) {
			...
			case "dazzlinggleam":
				return (
					!moves.includes("alluringvoice") ||
					this.formatType?.includes("doubles")
				);
			case "darkvoid":
				return dex.gen < 7;
			case "dualwingbeat":
			...
			case "hiddenpowergrass":
				return (
					!(dex.gen < 4 && moves.includes("leafblade")) ||
					(dex.gen > 3 &&
			...
			case "risingvoltage":
				return (
					abilityid === "electricsurge" || abilityid === "hadronengine"
				);
			case "rocktomb":
			...
			case "temperflare":
				return (
					(!moves.includes("flareblitz") &&
						!moves.includes("pyroball") &&
						!moves.includes("sacredfire") &&
						!moves.includes("bitterblade") &&
			...
			case "zapcannon":
				return (
					abilityid === "noguard" ||
					(dex.gen < 4 && !moves.includes("thunderwave"))
				);
		}
