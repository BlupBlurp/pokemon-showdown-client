...
export class TeamEditorState extends PSModel {
	...
	/** Returns the types the set currently uses, accounting for RelumiTesting custom types. */
	getDisplayTypes(set: Dex.PokemonSet, species: Species): readonly Dex.TypeName[] {
		if (this.isRelumiTesting && set.customTypes?.length) return set.customTypes;
	...
	renderDetails(set: Dex.PokemonSet, i: number) {
		...
		return <button class="textbox setdetails" name="details" value={i} onClick={this.clickDetails}>
			...
			{editor.gen === 9 && !editor.isChampions ? (
				<span class="detailcell">
					<label>Tera</label><PSIcon type={set.teraType || species.requiredTeraType || editor.getDisplayTypes(set, species)[0]} />
				</span>
					...
					<td colSpan={2} class="set-details"><div class="border-collapse">
						<button class={`button button-middle${cur('details')}`} onClick={this.setFocus} value={`set-${i}-details`}>
							...
							{editor.gen === 9 && !editor.isChampions && <span class="detailcell">
								...
								<PSIcon type={set.teraType || species.requiredTeraType || editor.getDisplayTypes(set, species)[0]} />
							</span>}
					...
					<td colSpan={2} class="set-details"><div class="border-collapse">
						<label class="label">
							...
							>
								...
								{editor.gen === 9 && !editor.isChampions && <span class="detailcell">
									<label>Tera</label> {}
									<PSIcon type={set.teraType || species.requiredTeraType || editor.getDisplayTypes(set, species)[0]} new={!editor.narrow} tera />
								</span>}
								{editor.hpTypeMatters(set) && <span class="detailcell">
									<label>H.P.</label> {}
...
class DetailsForm extends preact.Component<{
	...
	changeTera = (ev: Event) => {
		...
		if (!target.value || target.value === (species.requiredTeraType || displayTypes[0])) {
			delete set.teraType;
		} else {
					...
					<label class="label" title="Tera Type">
						...
						{species.requiredTeraType && editor.formeLegality === 'normal' ? (
							...
							<select
								...
								value={set.teraType || species.requiredTeraType || editor.getDisplayTypes(set, species)[0]}
							>
